...
const DeleteColumnModal = (props) => {
    const { setOpen, onConfirm, action, isLoading, columnName } = props;
    const ref = useRef();
                            ...
                            <div className="mt-2">
                                <div className="mb-4">
                                    <div className="text-center text-lg text-gray-400 text-opacity-75">
                                        {t(`¿Estás seguro de que deseas eliminar el campo: ${columnName} ?`)}
                                    </div>
                                </div>
                                ...
                                <div className="flex w-full flex-col justify-center pt-5 md:flex-row">
                                    ...
                                    <div className="mt-6 flex text-center md:mt-0">
                                        <button type="submit" className="button-primary w-32" onClick={onConfirm}>
                                            {isLoading ? <BeatLoader color={"white"} size={"0.625rem"} /> : t("Eliminar")}
                                        </button>
                                    </div>
                                </div>
                            </div>
