...
const TeamsTable = (props) => {
    const { deletePermission, handleDeleteTeam, handleViewTeam, handleActivateTeam, loading, maxPage, data, pageLimit, setPageLimit, nrows, setRows, lang, editPermission } = props;
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row: { original } }) => {
                    return <div className="overflow-hidden overflow-ellipsis font-semibold">{original.name}</div>;
                },
            },
            {
            ...
            {
                Header: t("AdminFilters.usersTotal"),
                accessor: (row) => get(row, "totalUsers"),
                Cell: ({ row: { original } }) => {
                    return <div className="ml-10 flex">{original.totalUsers}</div>;
                },
            },
            {
                ...
                Cell: ({ row: { original } }) => {
                    return (
                        <div className="flex items-center space-x-3">
                            {editPermission ? (
                                <div className="flex">
                                    <Tippy content={t("AdminFilters.editTeam")} theme={"tomato"} arrow={false} placement={"top"} touch={false}>
                                        <button className="text-gray-375 hover:text-gray-900" onClick={() => handleViewTeam(original)}>
                                ...
                                <div className="flex">
                                    <Tippy content={t("AdminFilters.viewTeam")} theme={"tomato"} arrow={false} placement={"top"} touch={false}>
                                        <button className="text-gray-375 hover:text-gray-900" onClick={() => handleViewTeam(original)}>
                            ...
                            {deletePermission && original.state ? (
                                <div className="flex">
                                    <Tippy content={t("teamsDelete.disableTeam")} theme={"tomato"} arrow={false} placement={"top"} touch={false}>
                                        <button
                                            className="text-gray-375 hover:text-gray-900"
                                            onClick={() => {
                                                handleDeleteTeam(original);
                                            }}
                                        >
                                ...
                                deletePermission && (
                                    <div className="flex">
                                        <Tippy content={t("AdminFilters.activateTeam")} theme={"tomato"} arrow={false} placement={"top"} touch={false}>
                                            <button className="mb-[0.15rem] text-gray-375 hover:text-gray-900" onClick={() => handleActivateTeam({ ...original, state: true })}>
    ...
    for (let i = 0; i < 6; i++) {
        loadingSkeleton.push(<TableSkeleton number={7} key={i} />);
    }
        ...
        return (
            <div className="rounded-b-xl border-t-[0.0938rem] border-gray-100 border-opacity-25 bg-white pt-3">
                <div className="overflow-x-auto">
                    <div className="inline-block min-w-full max-w-full justify-between bg-white align-middle">
                ...
                <div className="overflow-x-auto">
                    <div className="inline-block min-w-full max-w-full rounded-b-xl bg-white align-middle">
                        ...
                        <div className="flex justify-center">
                            <div className="flex h-72 items-center text-sm text-gray-400 text-opacity-75">{t("teamsForm.empty")}</div>
                        </div>
                    </div>
                </div>
                <div className="flex items-center justify-between rounded-b-xl border-t-2 border-gray-100/25 bg-white px-4 py-6"></div>
