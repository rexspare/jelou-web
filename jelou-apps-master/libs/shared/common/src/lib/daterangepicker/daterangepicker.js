...
const DateRangePicker = (props) => {
    const { dateChange, clearDate, dateValue, icon, background = "#fff", right = false, canDelete = true, placeholder = false, full } = props;
    ...
    return (
        <Popover className="relative">
            <div className={`flex ${full ? "w-full" : "min-w-5 lg:min-w-8"} flex-1`}>
                <Popover.Button className="flex flex-1 rounded-[0.8125rem] border-[0.0938rem] border-gray-100 border-opacity-50 px-[0.80rem] py-2 text-13 leading-5 text-gray-400 text-opacity-75">
                    ...
                    <span
                        ...
                        className={`label-top-animation flex gap-2 bg-white ${!isEmpty(dateValue) || isDate(endDate) ? "absolute translate-y-[-1.15rem]" : "translate-y-0"}`}
                    >
