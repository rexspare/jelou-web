...
import { CloseIcon, SearchIcon, CleanIcon, TimeIcon } from "@apps/shared/icons";
...
const GlobalSearchPma = (props) => {
    const { query, setQuery, searchButton, clean, typeSearchBy, setSearchBy, minLength = 2, type, position, placeholder } = props;
    ...
    return (
        <Popover className="flex items-center rounded-[1.0313rem] bg-white">
            <>
                <div className="relative w-full">
                    <Popover.Button
                        ref={searchButton}
                        className="w-full rounded-[0.8125rem] border-[0.0938rem] border-gray-100/50 py-1 pl-10 text-sm text-gray-darker placeholder:text-sm placeholder:text-gray-400 placeholder:text-opacity-70 focus:border-gray-100 focus:ring-transparent"
                    >
                        <div className="absolute inset-y-0 left-0 ml-5 flex items-center">
                            <SearchIcon className="h-4 w-5 fill-current sm:h-5 sm:w-5" />
                        </div>
                        <div className="mx-2 flex items-center space-x-2 truncate border-transparent py-1 text-gray-400 ring-transparent focus:border-transparent focus:ring-transparent">
                            {!isEmpty(input) ? (
                                <div className="flex">
                ...
                >
                    <Popover.Panel className={`${position ? position : "top-12 -ml-2 md:top-[0.3rem]"} absolute z-20 w-80 overflow-hidden rounded-xl bg-white shadow-preview mid:max-w-3xl`}>
                        ...
                        <div className={`mx-2 space-y-2 px-3 text-13 font-normal text-gray-400 ${input.length > 0 && input.length <= mnLength ? "pb-4" : "pt-3 pb-4"}`}>
                            {input.length > 0 && input.length <= mnLength && <span className="mt-1 flex justify-end text-10 italic text-red-675">*{t("Ingresar mas de 2 caracteres")}</span>}
                                    ...
                                    (search, key) =>
                                        key < 3 && (
                                            ...
                                            >
                                                ...
                                                {!isEmpty(search.field) && <span className="rounded-md bg-gray-400 bg-opacity-10 p-1 px-2 text-opacity-75">{`${t(get(search, "field.name"))}`}</span>}
                                ...
                                {isEmpty(input) ? (
                                    ...
                                    <div className="flex w-full items-center justify-between">
                                        <span className={`mr-2 flex items-center truncate text-sm ${input.length <= mnLength ? "text-gray-400" : "text-primary-200"}`}>
                                            ...
                                            {t("pma.Todos los resultados para")} <span className="max-w-xxs truncate font-bold italic">"{input}"</span>
                                        </span>
