...
import { DownloadIcon2, WarningIcon } from "@apps/shared/icons";
                ...
                <div className="mt-2">
                    <div className="text-center">
                        {castArray(quickReplies).map((option, index) => (
                            <span
                                className="mb-1 block rounded-lg border-1 border-primary-200 bg-white py-2 px-4 text-sm text-primary-200 shadow"
                                key={index}>
                                {option.title}
                            </span>
                        ))}
