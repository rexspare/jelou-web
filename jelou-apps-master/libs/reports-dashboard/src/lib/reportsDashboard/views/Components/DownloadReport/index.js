...
const Downloads = () => {
    ...
    useEffect(() => {
        const pusher = new Pusher("519345aac33f4a10ccf8", {
            cluster: "us2",
        });

        const channel = pusher.subscribe(`socket-${userSession.providerId}`);

        channel.bind("download-updated", function (data) {
        ...
        channel.bind("download-created", function (data) {
    ...
    return (
        <Menu as="div" className="relative flex h-full items-center justify-center">
            ...
            <Transition
                ...
                <Menu.Items className="absolute right-0 top-0 z-20 mt-14 flex max-h-[40rem] w-[400px] scale-100 transform  flex-col rounded-1 bg-white pt-6 opacity-100 shadow-dropdown focus:outline-none">
