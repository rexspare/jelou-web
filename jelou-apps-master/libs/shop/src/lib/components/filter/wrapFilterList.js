...
export function WrapFilterList({ arrayFilter = [], downloadData, isProduct = false, loadingdownload, setSelected } = {}) {
    const [valueLabel, setValueLabel] = useState("");
    const { t } = useTranslation();
    const handleDownloadDataTable = () => downloadData({ download: true });

    const { clearFilter: crearRangePriceFilter, handleChange, handleClick, valueRange, min_price, max_price } = useRange({ setSelected });

    const { dataRanges, dateValue, endDate, startDate, clearDate, onChange, dateChange, clearDateFields } = useDateFilter({
        setSelected,
    });

    const handleClearAllFilters = () => {
        crearRangePriceFilter();
        clearDateFields();
        setSelected({});
        setValueLabel("");
    };

    const clearComboBoxFilter = (filter) => {
        setSelected((preState) => {
            const { [filter]: _, ...rest } = preState;
            return rest;
        });

        setValueLabel((preState) => {
            const { [filter]: _, ...rest } = preState;
            return rest;
        });
    };
    ...
    const handleChangeComboBox = (selectedOption) => {
        ...
        setValueLabel((preState) => ({ ...preState, [key]: selectedOption }));
    };
            ...
            <div className="flex gap-4">
                {isProduct && (
                    <div className="block w-64">
                        <RangeFilter
                            ...
                            handleClearFilter={crearRangePriceFilter}
                ...
                {arrayFilter.length > 0 &&
                    arrayFilter.map((filter, i) => {
                        ...
                        return (
                            <div className="h-[2.4rem] w-64" key={filter.key + i}>
                                <ComboboxSelect
                                    options={filter.options}
                                    icon={filter.icon}
                                    value={valueLabel[filter.key] ?? ""}
                                    placeholder={filter.placeholder}
                                    label={filter.placeholder}
                                    handleChange={handleChangeComboBox}
                                    name={filter.key}
                                    background={"#fff"}
                                    clearFilter={clearComboBoxFilter}
                                />
                            </div>
                        );
                    })}
