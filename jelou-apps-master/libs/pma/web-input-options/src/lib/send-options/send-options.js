    ...
    return (
        <div>
            {showFileZone || hasText || !sendAudio || isMobileApp.AndroidApp() || isMobileApp.iOSApp() ? (
                ...
                <div className={`relative flex rounded-full ${record ? "RecTransition w-48 bg-primary-600" : `w-8 ${micPermission ? "bg-secondary-425" : "bg-secondary-400 "}`} h-8 justify-end`}>
                    {record ? (
                        ...
                        <Tippy content={t("pma.Audio Deshabilitado")} touch={false}>
                            <button className={`flex h-8 w-8 items-center justify-center border-transparent focus:outline-none`} onClick={() => askMicPermission()}>
                                <MicIcon className="fill-current text-white" fill="#ffffff" width="0.875rem" height="1.25rem" />
                            </button>
                        </Tippy>
                    )}
            ...
            <ReactMic record={record} className="hidden w-4" onStop={(evt) => onStop(evt)} strokeColor="#000000" backgroundColor="#209F8B" mimeType="audio/mp3" />
        </div>
    );
};
