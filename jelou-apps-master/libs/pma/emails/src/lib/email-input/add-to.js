...
function AddTo({ title, to, setTo, blackListEmails }) {
        ...
        if (!isEmpty(trim(itemLower))) {
            setTo((pre) => {
                if (!pre.includes(itemLower) && !blackListEmails.includes(itemLower)) {
                    ...
                    if (blackListEmails.includes(toLower(itemLower))) {
    ...
    const deleteCc = (index) => {
        const newCc = to.filter((item, i) => i !== index);
        setTo(newCc);
    };

    const disableTo = to.length > 3;
