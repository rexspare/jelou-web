...
const OpeningAnotherDraftModal = (props) => {
    const { onClose, onConfirm, isLoading = false } = props;

    const nextDraftRoom = useSelector((state) => state.nextDraftRoom);

    const nextDraftNumber = get(nextDraftRoom, "number", null);
    ...
    return (
        <div className="fixed inset-x-0 top-0 z-[130] overflow-auto px-4 pt-8 sm:inset-0 sm:flex sm:items-center sm:justify-center sm:p-0">
            ...
            <div ref={modalRef} className="relative z-70 min-h-20 w-[24rem] rounded-xl bg-white px-6 pt-6 pb-6">
                <span className="absolute right-4 top-4" onClick={() => onClose()}>
                ...
                <div className="flex space-x-2">
                    ...
                    <p className="flex-1 font-bold text-[#EC5F4F]">Estás intentando abrir otro borrador</p>
                </div>
                <div className="mt-3 flex flex-col text-sm">
                    <p className="flex-1 text-gray-500">
                        {`¿Deseas guardar el actual borrador y abrir el borrador con numero `}
                        <span className="font-bold text-primary-200">{nextDraftNumber}?</span>{" "}
                    </p>
                    <p className="pt-2 font-bold text-gray-400">¿Deseas hacerlo?</p>
                </div>
                <div className="flex justify-end space-x-3 pt-2">
                    <div className={`hidden items-center lg:flex`}>
                        <button
                            ...
                            onClick={() => onClose()}>
                            <span className="flex items-center justify-center text-sm font-bold text-gray-400">Regresar</span>
                        </button>
                    </div>
                    <div className={`hidden items-center lg:flex`}>
                        <button
                            ...
                            onClick={() => onConfirm()}>
                            <span className="group-hover:text-primary-hover w-full text-15 text-white">
                                {isLoading ? (
...
export default OpeningAnotherDraftModal;