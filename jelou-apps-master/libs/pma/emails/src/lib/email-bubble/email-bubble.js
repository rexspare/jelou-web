...
const EmailBubble = (props) => {
    const { isLast = false, message, scrollUpRef, key } = props;
                    ...
                    <div className="mail-content w-full max-w-full overflow-hidden px-10 pt-6 pb-10">
                        ...
                        {!isEmpty(attachments) && (
                            <div className="flex flex-col">
                                <span className="text-13 font-bold text-gray-500">{t("pma.Archivos Adjuntos:")}</span>
                            </div>
                        )}
                    </div>
                );
    ...
    return (
        <div className="z-1 top-0 w-full rounded-12 bg-white px-3 md:px-5 md:pt-5 md:pb-3" ref={scrollUpRef}>
            <button
                ...
                {show && toUpper(by) === "OPERATOR" && (
                    <div
                        className={
                            "w-full border-b-default border-solid border-gray-100 border-opacity-25 pb-3 text-left text-15 text-gray-400 md:pb-4"
                        }>
                        {t(`pma.Contestado por`)} <b>{get(sender, "names", t("pma.Desconocido"))}</b>
                    </div>
                )}
                <div className={`mb-2 flex items-center ${show && toUpper(by) === "OPERATOR" ? "pt-4 md:pt-5" : ""}`}>
                    <RoomAvatar src={"https://s3.us-west-2.amazonaws.com/cdn.devlabs.tech/default_avatar.jpeg"} name={name} />
                ...
                <div className={`${show ? "flex items-center justify-end space-x-2" : "flex w-full justify-between"}`}>
                    {!show && <span className={"flex-1 text-left text-sm text-gray-400"}>{previewHtml}</span>}
                </div>
            </button>
            {show && !isEmpty(html) && (
                <ErrorBoundary>
                    <div className="mail-content h-auto w-full max-w-full overflow-hidden px-10 pt-6 pb-10">
                        {
                            <iframe
                                loading="lazy"
                                srcDoc={html}
...
const MemoizedEmailBubble = React.memo(EmailBubble);

export default MemoizedEmailBubble;
