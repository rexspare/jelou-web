// import styles from "./plugins-datafast-pruebas.module.scss";
...
export function DatafastPruebas(props) {
    ...
    useEffect(() => {
        const { id, senderId, appId } = props.currentRoom;
  ...
  */
    ...
    const resolveCase = async (ticket, senderParamsObj) => {
        ...
        try {
            const { data } = await axios.post("http://18.237.121.197:8080/datachat/Chatbot_ResolverCaso?environment=development", res);
            ...
            try {
                ...
                await JelouApiV1.post("/v1/company/284/databases/81/rows", params, config);
            } catch (error) {
                console.log("error: ", error);
            }
    ...
    const asignCase = async () => {
        let obj = {
            ...
            //origen: "OC",
            origen: get(props.storeParams, "_canal", "") === "WEB" ? "OCW" : "OC",
        ...
        try {
            ...
            Ticket = get(data, "ChatBot_RegistrarCasoResult.Ticket", "");
            let errorMessage = "";
            let errorTrace = "";
            ...
            if (storeParams.accionType === "RESOLVE" && Ticket !== null) {
                ...
                setLoading(false);
                try {
                    await JelouApiV1.post("/v1/company/284/databases/81/rows", senderParamsObj, config);
                } catch (error) {
                    console.log("error: ", error);
                }
            }
