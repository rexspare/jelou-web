...
function PorHilo(props) {
    const { data, selected, handleSelectAllClick, inputCheckboxClassName, t, isSelected, handleClick, srcVacio, bot } = props;
                ...
                <thead className="flex w-full items-center justify-start border-b-2 border-gray-34 text-gray-700">
                    <tr className="flex items-center text-center">
                        <th scope="col" className="px-3 py-4">
                            <input checked={data.length > 0 && selected.length === data.length} onChange={handleSelectAllClick} type="checkbox" className={inputCheckboxClassName} />
                        </th>
                        <th scope="col" className=" px-3 py-4">
                            <div className="flex w-[20vw] items-center  justify-between ">
                                <h4 className="font-bold text-gray-400">
                                    {selected.length + " "}
                                    {t("MassAchivePost.titleResult")}
                                </h4>
                                {/* <button>
                                            <MenuDoot />
                                        </button> */}
                            ...
                            <tr className="border-b-2 border-gray-34 bg-white" key={sender.referenceId}>
                                <td className="px-3 py-4">
                                    <input
                                        ...
                                        id="mergeBubbles"
                                        onChange={(evt) => handleClick(evt, messageId)}
                                        name="mergeBubbles"
                                        type="checkbox"
