...
import { CloseIcon, OperatorIcon2 } from "@apps/shared/icons";
import { HistoryEvent } from "../history-event/history-event";

const HistoryTickets = (props) => {
    const { t, currentEmail } = props;
    ...
    return (
        <>
            <Tippy theme={"tomato"} content={t("Historial")} arrow={false}>
                    ...
                    <div className="relative min-h-screen">
                        ...
                        <Transition.Child
                            ...
                            <div className="absolute top-0 right-0 my-2 mr-2 h-history w-72 transform overflow-y-scroll rounded-xl bg-white py-9 pl-8 pr-3 shadow-xl transition-all">
                                <Dialog.Title as="h3" className="mb-4 flex justify-between text-2xl font-bold leading-6 text-primary-200">
                                    <span>Historial</span>
                                    <button onClick={closeModal}>
                                        <CloseIcon fill="#727C94" className="text-gray-475" width="0.953125em" height="0.954375em" />
                                    </button>
                                </Dialog.Title>
                                ...
                                <Dialog.Description>
                                    {Boolean(lifecycle) && lifecycle.length ? (
                                        ...
                                        <p className="text-center text-gray-15">No hay eventos registrados</p>
                                    )}
                                    <section>
                                        <h3 className="my-4 text-2xl font-bold leading-6 text-primary-200">Asignaciones</h3>
                                        {assignationFlow ? (
                                            [...assignationFlow].reverse().map((assignation, i) => {
                                                ...
                                                return (
                                                    <aside key={id + i}>
                                                        <div className="ml-3">
                                                            <p className="text-15 text-gray-475 text-opacity-65">
                                                                <span className="text-gray-475">
                                                            ...
                                                            <p className="flex gap-2 text-base font-normal text-gray-475">
                                                                <span>{dateAt}</span>
                                                                <span>{timeAt}</span>
                                                            </p>
                                                        </div>
                                            ...
                                            <p className="text-center text-gray-15">No hay asignaciones registradas</p>
                                        )}
                                    </section>
                                </Dialog.Description>
                            </div>
...
export default withTranslation()(HistoryTickets);
