...
const HsmTable = (props) => {
    ...
    const color = (values) => {
        ...
        if (toUpper(values) === "DELIVERED_CHANNEL") {
            return "text-stat-total bg-stat-bgTotal";
        }
    ...
    const Badge = ({ values }) => {
        ...
        // DELIVERED_USER FAILED DELIVERED_CHANNE
        switch (sentStatus) {
            case "FAILED":
                return (
    ...
    const columns = useMemo(
        () => [
            ...
            {
                Header: t("HSMTable.answered"),
                accessor: (row) => {
                    return truncate(get(row, "message", "--"), {
                        length: "25",
                    });
                },
            },
