...
const MessagePreview = (props) => {
    ...
    try {
        switch (toUpper(type)) {
            case "TEXT":
                var truncate_user = `${text.length > 20 ? "w-40 xl:w-48" : ""} truncate text-13 sm:text-sm block text-gray-400 font-medium`;

                return (!isEmpty(props.field) && props.field.type === "text") || props.field.type === "client" ? (
...
const Room = (props) => {
    const { room, setIsLoadingMessage, query, field, selectedOptions, setLoadingOperators } = props;
    ...
    const getMessages = async () => {
        ...
        const date = selectedOptions.date;
                ...
                <div className="relative flex flex-row items-center">
                    ...
                    <div className="flex w-full flex-col space-y-1">
                        <div className="flex items-center justify-between text-sm sm:text-15">
                            {(!isEmpty(field) && field.type === "text") || field.type === "client" ? (
